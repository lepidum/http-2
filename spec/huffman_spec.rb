  ...
  context "decode" do
    before(:all) { @encoder = HTTP2::Header::Huffman.new }
      ...
      it "should raise when input is shorter than expected" do
        plain, encoded = huffman_examples[0]
        encoded = [encoded].pack("H*")
        expect { @encoder.decode(HTTP2::Buffer.new(encoded[0...-1]), plain.bytesize) }.to raise_error CompressionError
      end
      it "should raise when input is not padded by 1s" do
        ...
        encoded = [encoded].pack("H*")
        expect { @encoder.decode(HTTP2::Buffer.new(encoded), plain.bytesize) }.to raise_error CompressionError
      end
    end
  end
end
